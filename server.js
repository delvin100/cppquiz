...
const connectToMongoDB = async () => {
  try {
    ...
    await mongoose.connect('mongodb+srv://delvinvarghese2028:Delvin2806@cluster0.isgjvem.mongodb.net/quizdb?retryWrites=true&w=majority&appName=Cluster0');
...
const authMiddleware = (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token.split(' ')[1], 'mysecret123');
...
app.post('/signup', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ user: { id: user.id } }, 'mysecret123');
    res.json({ token });
  } catch (err) {
